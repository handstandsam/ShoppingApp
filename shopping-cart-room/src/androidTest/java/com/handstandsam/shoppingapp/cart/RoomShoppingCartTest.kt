...
class RoomShoppingCartTest {
  ...
  fun setUp() {
    ...
    Handler(Looper.getMainLooper()).post {
      testDelegate = RoomShoppingCartTestDelegate().apply {
        runBlocking(Dispatchers.IO) { clearDb() }
      }
      latch.countDown()
    }
  ...
  fun happyPath() = runBlocking<Unit> {
    testDelegate
      ...
      .assertPersisted(item1, expectedQuantity = 1)
      .assertPersisted(item2, expectedQuantity = 1)
      .assertTotalItemsInCart(2, 2)
      .incrementItemInCart(item1)
      .assertPersisted(item1, expectedQuantity = 2)
